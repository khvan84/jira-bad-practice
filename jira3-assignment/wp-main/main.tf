...
module "wp-ec2" {
  source = "../wp-pub/wordpress-ec2"
most_recent         = true
virtualization_type = "virtualization-type"
key_name            = "linuxkey"
instance_type       = "t2.micro"
ec2_name            = "wordpress-EC2"
subnet_id = module.wp-pub.subnet_ids[0]
EC2_SG_ID = module.wp-pub.db-wp-sg-id.id
user_data = <<-EOF
              #!/bin/bash
              sudo yum update -y
...
module "mysql" {
  ...
  subnet_id = module.wp-priv.subnet_ids
  rds-sg_id = module.wp-priv.rds-sg_ids.id
}
